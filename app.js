...
function openModal(title, desc) {
  document.getElementById("modalTitle").innerText = title;     // Title ko plain text rakho
  document.getElementById("modalDesc").innerHTML = desc;       // Yaha innerHTML use karo
...
// our services
  ...
  function flipCard(buttonElement) {
    // Find closest card container (assumes structure above)
    const wrapper = buttonElement.closest('.h-96, [class*="md:min-w"]');
    if (!wrapper) return;
    ...
    // Optional: if flipped to back, focus the Back button for accessibility
    if (inner.classList.contains('flipped')) {
      const backBtn = wrapper.querySelector('.back button');
      if (backBtn) backBtn.focus();
    } else {
      const frontBtn = wrapper.querySelector('.front button');
